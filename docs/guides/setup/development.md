...
4. [Архитектура Middleware](#архитектура-middleware)
5. [Система авторизации](#система-авторизации)
6. [Система логирования](#система-логирования)
7. [Обработка запросов](#обработка-запросов)
...
При разработке проекта FinPuls придерживайтесь следующих принципов:

### DRY (Don't Repeat Yourself)

**Не повторяйся** — избегайте дублирования кода.

**Правила:**
- Выносите повторяющийся код в методы или утилитные классы
- Используйте наследование и композицию для переиспользования
- Создавайте общие компоненты для повторяющейся функциональности
...
public class UserService {
    private <T> void validateNotNull(T value, String fieldName) {
        if (value == null) {
...
**Делай проще** — простота важнее сложности.

**Правила:**
- Пишите простой и понятный код
- Избегайте излишней абстракции
- Решайте проблемы простыми способами
...
**Тебе это не понадобится** — не добавляйте функциональность "на будущее".

**Правила:**
- Реализуйте только то, что нужно сейчас
- Не добавляйте код "на всякий случай"
- Не создавайте абстракции до того, как они действительно понадобятся
- Рефакторите код, когда появляется реальная необходимость
...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    List<User> findByStatusAndRoleAndCreatedAfter(
        UserStatus status, 
        UserRole role, 
        LocalDateTime createdAfter
    );
    
    // Еще не нужно, но "на будущее"
...
**При написании кода:**
1. **DRY** — проверьте, не дублируется ли код
2. **KISS** — убедитесь, что решение максимально простое
3. **YAGNI** — реализуйте только необходимую функциональность

**При рефакторинге:**
- Если код повторяется → примените DRY
- Если код слишком сложный → примените KISS
- Если есть неиспользуемый код → примените YAGNI (удалите)
